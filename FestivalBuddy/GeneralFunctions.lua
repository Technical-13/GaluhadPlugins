...
function GetString(stringID,langID) -- langID not essential, will take loaded value if missing.
	if stringID == nil then return "" end;
	if langID == nil then langID = LangID end;
	-- this function checks for the string id in the prefered language
	-- if it can't find in the prefered language then it will search in the default language (English)
	-- if it can't be found still, then string error is returned.
	if Strings[langID] ~= nil and Strings[langID][stringID] ~= nil then return Strings[langID][stringID] end;
	if Strings[1] ~= nil and Strings[1][stringID] ~= nil then return Strings[1][stringID] end;
	return "<string error id:" .. stringID .. ">";
end
