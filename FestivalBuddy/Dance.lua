...
-- Main Parent window to hold all the elements
	...
	wDanceParent:SetText( _LANG.BUTTONS.DANCE[ SETTINGS.LANGUAGE ] );
	...
	qsDanceOne:SetShortcut( Turbine.UI.Lotro.Shortcut( Turbine.UI.Lotro.ShortcutType.Alias, "/dance1" ) );
	qsDanceTwo:SetShortcut( Turbine.UI.Lotro.Shortcut( Turbine.UI.Lotro.ShortcutType.Alias, "/dance2" ) );
	qsDanceThree:SetShortcut( Turbine.UI.Lotro.Shortcut( Turbine.UI.Lotro.ShortcutType.Alias, "/dance3" ) );
...
-- This function filters the dance step instructions and highlights the relevant box.
	...
	for k,v in pairs ( _DANCESTRINGS ) do
		local STRING = v[ SETTINGS.LANGUAGE ];
...
-- Target checking
	...
	if SELTARGET ~= nil then
		...
		if TARGETNAME ~= INSTRUCTORNAME then
			local tmpMsg = Alert( INSTRUCTORNAME .. _LANG.OTHER.EMOTETARGET[ SETTINGS.LANGUAGE ] );
		end
	else
		local tmpMsg = Alert( INSTRUCTORNAME .. _LANG.OTHER.EMOTETARGET[ SETTINGS.LANGUAGE ] );
	end
