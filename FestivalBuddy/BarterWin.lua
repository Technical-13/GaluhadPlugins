function DrawBarterWin()
	wBarterWinParent = Turbine.UI.Lotro.Window();
		...
		wBarterWinParent:SetText(_LANG.OTHER.BARTER[SETTINGS.LANGUAGE   ] );
...
-- Events
	...
	wBarterWinParent.Closing = function()
		SETTINGS.BARTERWIN.VISIBLE = false; end
	wBarterWinParent.VisibleChanged = function( Sender, args )
		if wBarterWinParent:IsVisible() == true then wBarterWinParent:Activate() end; end
end
function RefreshBarterList()
	...
	lblBarterFestival:SetText( _LANG.OTHER.BARTERITEMS[ SETTINGS.LANGUAGE ] .. ": " .. _LANG.FESTIVALS[ SELECTEDFESTIVAL ][ SETTINGS.LANGUAGE ] );
