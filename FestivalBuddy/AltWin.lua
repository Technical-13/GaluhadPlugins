function DrawAltWin()
	...
	if _ALTS[1] ~= nil then
		...
		btnRefreshAltDisplay.MouseEnter = function ( Sender, args )
			btnRefreshAltDisplay:SetBackground( _IMAGES.REFRESH_OVER ); end
		btnRefreshAltDisplay.MouseLeave = function ( Sender, args )
			btnRefreshAltDisplay:SetBackground( _IMAGES.REFRESH_NORMAL ); end
		btnRefreshAltDisplay.MouseClick = function ( Sender, args )
			RefreshAltDisplay( ddAltNames:GetText() ); end
		...
		lblNoAlts = Turbine.UI.Label();
			...
			lblNoAlts:SetText( _LANG.OTHER.NOALTS[ SETTINGS.LANGUAGE ] );
	end
	-- Window events
	wAltWinParent.Closing = function()
		SETTINGS.ALTWIN.VISIBLE = false; end
